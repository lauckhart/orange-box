/**
  ...
  Post processor configuration for DDCS v2.1.  Derived from GRBL revision below.
...
var settings = {
  ...
  retract: {
    ...
    }, // Specifies when XY should be homed in XY (sample: onIndexing:[X,Y]). Options can be combined
  },
  machineAngles: {
...
groupDefinitions.machineSimulation = {
...
properties.simulateConnectionMoves = {
  ...
  description:
    "Specifies that connection moves like prepositioning, tool changes, retracts and other non-cutting moves should be shown in the machine simulation." +
    EOL +
...
function machineSimulation(parameters) {
  if (
    revision < 50075 ||
    skipBlocks ||
    !getProperty("simulateConnectionMoves")
  ) {
  ...
  if (
    mode !== undefined &&
    ![TCPON, TCPOFF, TWPON, TWPOFF, TOOLCHANGE].includes(mode)
  ) {
  ...
  var enableTCP =
    mode != undefined
      ? mode == TCPON
      : typeof state !== "undefined" && state.tcpIsActive;
  var enableTWP =
    mode != undefined
      ? mode == TWPON
      : typeof state !== "undefined" && state.twpIsActive;
  var disableTCP =
    mode != undefined
      ? mode == TCPOFF
      : typeof state !== "undefined" && !state.tcpIsActive;
...
function setWorkPlane(abc) {
  ...
  writeStartBlocks(workplaneIsRequired, function () {
    ...
    if (
      currentSection.getId() > 0 &&
      isTCPSupportedByOperation(
        getSection(currentSection.getId() - 1) || tcp.isSupportedByOperation
      ) &&
